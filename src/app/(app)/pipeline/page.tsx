...
function DroppableColumn({ id, collapsed, children }: { id: string; collapsed?: boolean; children: React.ReactNode }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`${collapsed ? "w-[100px] flex-shrink-0" : "flex-1 min-w-[280px]"} rounded-xl2 p-2 transition-all ${isOver ? "bg-calume-navy/5" : ""}`}
    >
      {children}
    </div>
  );
              ...
              <DroppableColumn id={FAVORITES_ID} collapsed={collapsed[FAVORITES_ID]}>
                {collapsed[FAVORITES_ID] ? (
                  ...
                  >
                    ...
                    <Star size={32} className="fill-calume-gold text-calume-gold" />
                    <span className="text-2xl font-medium bg-gray-100 rounded-full px-2 py-0.5">{favoriteItems.length}</span>
                    <span className="text-2xl text-calume-gold font-semibold [writing-mode:vertical-rl] rotate-180 mt-1">
                      Favoritos
                    </span>
                  </button>
                ) : (
                ...
                return (
                  <DroppableColumn key={stage} id={stage} collapsed={collapsed[stage]}>
                    {collapsed[stage] ? (
                      ...
                      >
                        <ChevronRight size={32} className="text-gray-400" />
                        <span className="text-2xl font-medium bg-gray-100 rounded-full px-2 py-0.5">{items.length}</span>
                        {alertCount > 0 && stage !== "GANADO" && stage !== "PERDIDO" && (
                          <span className="text-2xl text-alert-red font-medium">⚠</span>
                        )}
                        <span
                          className="text-2xl font-semibold [writing-mode:vertical-rl] rotate-180 mt-1"
